...
nonisolated public enum TrackerModuleLibrary {
    public static var supportedExtensions: [String] {
        MusicSettings.current.trackerModuleExtensions
    }
    
    public static var trackerModuleDirectory: String {
        MusicSettings.current.trackerModuleDirectory
    }

    public static func discover(in bundle: Bundle = .main) -> [TrackerModuleTrack] {
        var urls = Set<URL>()
        let moduleDirectory = trackerModuleDirectory
